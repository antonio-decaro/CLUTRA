...
__device__ void
StealerDevice::init(clutra::detail::utils::SharedQueue<BlockSize> *,
...
template <size_t BlockSize>
__device__ void StealerDevice::steal(SharedState<BlockSize> &, int, uint32_t &,
                                     uint32_t &) const {}

template <size_t BlockSize> __device__ void StealerDevice::finalize() const {}

template <size_t BlockSize>
__device__ void StealerDevice::setReady(SharedState<BlockSize> &, bool) const {}

template <size_t BlockSize>
__device__ void BasicStealerDevice::init(
    clutra::detail::utils::SharedQueue<BlockSize> *local_queue,
...
template <size_t BlockSize>
__device__ int
BasicStealerDevice::attemptStealing(SharedState<BlockSize> &state,
  ...
  cg::invoke_one(block, [&]() {
    ...
    while (true) {
      ...
      for (int16_t victim_offset = 1; victim_offset < cluster.dim_blocks().x;
           ++victim_offset) {
        int16_t potential_victim_rank =
            (cluster.block_rank() + victim_offset) % cluster.dim_blocks().x;
        ...
        auto *victim_finished_ptr =
            state.is_finished_ptr[potential_victim_rank];
        ...
        cuda::atomic_ref<int, cuda::thread_scope_device> head_ref(
            victim_queue->head);
        cuda::atomic_ref<int, cuda::thread_scope_device> tail_ref(
            victim_queue->tail);
        const int tail_snapshot = tail_ref.load(cuda::memory_order_relaxed);
        if (tail_snapshot - head_ref.load(cuda::memory_order_relaxed) >
            chunk_size) {
          if (atomicCAS(&(victim_queue->tail), tail_snapshot,
                        tail_snapshot - chunk_size) != tail_snapshot) {
...
template <size_t BlockSize>
__device__ void BasicStealerDevice::steal(SharedState<BlockSize> &state, int i,
                                          uint32_t &vertex,
...
#define INSTANTIATE_STEALER_DEVICE_TEMPLATES(BlockSize)                        \
  ...
  template __device__ void StealerDevice::init<BlockSize>(                     \
      clutra::detail::utils::SharedQueue<BlockSize> *,                         \
      StealerDevice::SharedState<BlockSize> &) const;                          \
  template __device__ void BasicStealerDevice::init<BlockSize>(                \
      clutra::detail::utils::SharedQueue<BlockSize> *,                         \
      BasicStealerDevice::SharedState<BlockSize> &) const;                     \
  template __device__ int StealerDevice::attemptStealing<BlockSize>(           \
      StealerDevice::SharedState<BlockSize> &, int) const;                     \
  template __device__ int BasicStealerDevice::attemptStealing<BlockSize>(      \
      BasicStealerDevice::SharedState<BlockSize> &, int) const;                \
  template __device__ void StealerDevice::steal<BlockSize>(                    \
      StealerDevice::SharedState<BlockSize> &, int, uint32_t &, uint32_t &)    \
      const;                                                                   \
  template __device__ void BasicStealerDevice::steal<BlockSize>(               \
      BasicStealerDevice::SharedState<BlockSize> &, int, uint32_t &,           \
      uint32_t &) const;                                                       \
  ...
  template __device__ void StealerDevice::setReady<BlockSize>(                 \
      StealerDevice::SharedState<BlockSize> &, bool) const;                    \
  template __device__ void BasicStealerDevice::setReady<BlockSize>(            \
