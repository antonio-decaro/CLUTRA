...
struct ActiveFrontierStatus {
  ...
  __host__ bool isComputed(bool invert) const { return invert ? inverted_computed : normal_computed; }
  ...
  __host__ bool isAnyComputed() const { return inverted_computed || normal_computed; }
};
...
public:
  ...
  static constexpr uint32_t alignUp(uint32_t value) { return (value + alignment - 1) / alignment * alignment; }
...
#pragma unroll
    for (uint16_t i = 0; i < Levels; i++) {
      T lidx = idx;
      for (uint16_t _ = 0; _ < i; _++) {
        lidx /= _range;
      }  // the index must be divided by the range^level
      if (!(_data[i][getBitmapIndex(lidx)] & (static_cast<bitmap_type>(1) << (lidx % _range)))) {
        atomicOr(&_data[i][getBitmapIndex(lidx)],
                 static_cast<bitmap_type>(static_cast<bitmap_type>(1) << (lidx % _range)));
      }
    }
  }

  __device__ inline void remove(uint32_t idx) const {
    atomicAnd(&_data[0][getBitmapIndex(idx)],
              ~(static_cast<bitmap_type>(static_cast<bitmap_type>(1) << (idx % _range))));
    for (uint16_t i = 1; i < Levels; i++) {
      uint32_t lidx = idx;
      for (uint16_t _ = 0; _ < i; _++) {
        lidx /= _range;
      }  // the index must be divided by the range^level
      atomicAnd(&_data[i][getBitmapIndex(lidx)],
                ~(static_cast<bitmap_type>(static_cast<bitmap_type>(1) << (lidx % _range))));
    }
  }
...
#pragma unroll
    for (uint16_t l = 0; l < Levels; ++l) {
      for (uint32_t i = 0; i < _size[l]; ++i) {
        _data[l][i] = static_cast<bitmap_type>(0);
      }
    }
  }
  ...
  __device__ inline bool check(uint32_t idx) const {
    return _data[0][idx / _range] & (static_cast<bitmap_type>(1) << (idx % _range));
  }

  __device__ inline bool empty() const {
    bitmap_type count = static_cast<bitmap_type>(0);
    for (auto i = 0; i < _size[Levels - 1]; i++) {
      count += _data[Levels - 1][i];
    }
