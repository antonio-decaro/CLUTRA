...
template <advance_direction Direction,
          graph::detail::DeviceGraphConcept GraphDevT, typename FrontierDevT,
          typename LambdaT>
__device__ __forceinline__ void
processVertexRange(GraphDevT graph_dev, FrontierDevT out_dev_frontier,
                   LambdaT functor, uint32_t vertex, uint32_t degree,
                   uint32_t lane, uint32_t stride) {
  auto start = graph_dev.begin(vertex);
  for (uint32_t edge_offset = lane; edge_offset < degree;
       edge_offset += stride) {
...
__device__ uint32_t getAssignedVertex(const FronterDevT &in_dev_frontier,
                                      uint32_t coarsening_factor, uint32_t gid,
                                      uint32_t tid) {
  if constexpr (View == view::graph) {
    ...
    const uint32_t actual_id_offset =
        (gid * coarsening_factor) + (tid / bitmap_range);
    uint32_t assigned_vertex;
    if (actual_id_offset < offsets_size) {
      assigned_vertex = (bitmap_offsets[actual_id_offset] * bitmap_range) +
                        (tid % bitmap_range);
    } else {
...
template <view View, advance_direction Direction,
          graph::detail::DeviceGraphConcept GraphDevT, typename FrontierDevT>
__device__ __forceinline__ bool
checkVertexActive(const GraphDevT &graph_dev,
                  const FrontierDevT &in_dev_frontier, uint32_t vertex) {
  if constexpr (View == view::frontier) {
    if constexpr (Direction == advance_direction::push) {
      return (vertex < graph_dev.getVertexCount()) &&
             in_dev_frontier.check(vertex);
    } else {
      return (vertex < graph_dev.getVertexCount()) &&
             !in_dev_frontier.check(vertex);
    }
...
template <advance_direction Direction, size_t BlockSize,
          graph::detail::DeviceGraphConcept GraphDevT, typename OutFrontierDevT,
          typename StealerDeviceT, typename LambdaT>
__device__ __forceinline__ void runLocalStealLoop(
    GraphDevT graph_dev, OutFrontierDevT out_dev_frontier,
    StealerDeviceT stealer,
    typename StealerDeviceT::template SharedState<BlockSize> &stealer_state,
    LambdaT functor, int tid, int block_dim) {
  while (true) {
    const int steal_count = stealer.template attemptStealing<BlockSize>(
        stealer_state, stealer.getStealingChunkSize());
    if (steal_count == 0) {
      break;
    }
    ...
    for (int i = 0; i < steal_count; ++i) {
      stealer.template steal<BlockSize>(stealer_state, i, steal_vertex,
                                        steal_degree);
      processVertexRange<Direction>(graph_dev, out_dev_frontier, functor,
                                    steal_vertex, steal_degree, tid, block_dim);
    }
    __syncthreads();
  }
}
...
size_t getAdvanceSharedMemorySize(size_t stealer_shared_size) {
  ...
  // CTA queue
  shared_size +=
      clutra::detail::utils::SharedQueue<BlockSize>::getSizeInBytes();
  // Warp queues
  shared_size +=
      (BlockSize / WARP_SIZE) *
      clutra::detail::utils::SharedQueue<WARP_SIZE>::getSizeInBytes();
  // Stealer shared state
  shared_size += stealer_shared_size;
  return shared_size;
}

template <view View, advance_direction Direction, size_t BlockSize,
          graph::detail::DeviceGraphConcept GraphDevT, typename InFrontierDevT,
          typename OutFrontierDevT, typename StealerDeviceT, typename LambdaT>
__global__ void
advanceKernel(GraphDevT graph_dev, InFrontierDevT in_dev_frontier,
              OutFrontierDevT out_dev_frontier, int coarsening_factor,
              size_t total_iters, StealerDeviceT stealer, LambdaT functor) {
  constexpr int WARP_SIZE = 32;
  static_assert(BlockSize % WARP_SIZE == 0,
                "BlockSize must be multiple of warp size");

  __shared__ clutra::detail::utils::SharedQueue<BlockSize> cta_queue;
  __shared__ clutra::detail::utils::SharedQueue<WARP_SIZE>
      warp_queues[BlockSize / WARP_SIZE];
  __shared__ typename StealerDeviceT::template SharedState<BlockSize>
      stealer_state;
  ...
  for (size_t iter = 0; iter < total_iters;
       ++iter) { // TODO change with stealing with ptx (only sm_100)
    const uint32_t tile_gid =
        static_cast<uint32_t>(blockIdx.x + (iter * gridDim.x));
    ...
    const uint32_t assigned_vertex = getAssignedVertex<View>(
        in_dev_frontier, coarsening_factor, tile_gid, tid);
    const bool vertex_active = checkVertexActive<View, Direction>(
        graph_dev, in_dev_frontier, assigned_vertex);
    if (vertex_active) {
    ...
    while (cta_queue.pop(vertex, degree)) {
      processVertexRange<Direction>(graph_dev, out_dev_frontier, functor,
                                    vertex, degree, tid, block_dim);
      __syncthreads();
    }

    for (int i = 0; i < warp_queue.size(); ++i) {
      processVertexRange<Direction>(graph_dev, out_dev_frontier, functor,
                                    warp_queue.vertices[i],
                                    warp_queue.degrees[i], lane, WARP_SIZE);
    }
  }
  if (!stealer.isStealingEnabled()) {
    return;
  ...
  runLocalStealLoop<Direction, BlockSize>(graph_dev, out_dev_frontier, stealer,
                                          stealer_state, functor, tid,
                                          block_dim);
...
template <size_t BlockSize, graph::detail::DeviceGraphConcept GraphDevT,
          typename StealerDeviceT, typename LambdaT>
__global__ void advanceKernel(GraphDevT graph_dev, size_t total_iters,
                              StealerDeviceT stealer, LambdaT functor) {
  ...
  for (size_t iter = 0; iter < total_iters;
       ++iter) { // TODO change with stealing with ptx
    const uint32_t tile_gid =
        static_cast<uint32_t>(blockIdx.x + (iter * gridDim.x));
  ...
  while (true) {
    ...
    uint32_t steal_degree = 0;
    for (int i = 0; i < steal_count; ++i) {
      stealer.template steal<BlockSize>(stealer_state, i, steal_edge,
                                        steal_degree);
