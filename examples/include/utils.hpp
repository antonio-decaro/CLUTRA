...
struct CLIHandles {
  ...
  CLI::Option* global_stealing_chunk_size_opt = nullptr;
...
inline CLIHandles configureBaseCLI(CLI::App& app, Options& opts) {
  ...
  app.add_flag("--measure-work", opts.measure_work, "Measure the amount of work done by the algorithm (number of edges traversed)");
