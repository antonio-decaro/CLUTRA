...
bool validate(const GraphT& graph, float* device_distances, uint source) {
  ...
  std::priority_queue<std::pair<uint32_t, float>, std::vector<std::pair<uint32_t, float>>, Prioritize<uint32_t, float>>
      pq;
  ...
  for (auto i = 0; i < graph.getVertexCount(); i++) {
    if (distances[i] != device_distances[i]) {
      std::cerr << "Mismatch at vertex " << i << " | Expected: " << distances[i] << " | Got: " << device_distances[i]
                << std::endl;
      return false;
    }
  }
